...
// deepCopyMethod returns the signature of a DeepCopy() method, nil or calls glog.Fatalf
...
// or:
//    func (t *T) DeepCopy() *T
func deepCopyMethod(t *types.Type) *types.Signature {
...
// deepCopyIntoMethod returns the signature of a DeepCopyInto() method, nil or calls glog.Fatalf
// if the type is wrong. DeepCopyInto allows more efficient deep copy
// implementations to be defined by the type's author.  The correct signature
// for a type T is:
//    func (t T) DeepCopyInto(t *T)
// or:
//    func (t *T) DeepCopyInit(t *T)
func deepCopyIntoMethod(t *types.Type) *types.Signature {
...
func copyableType(t *types.Type) bool {
	...
	if t.Kind == types.Alias {
		...
		if deepCopyMethod(t) != nil || deepCopyIntoMethod(t) != nil {
			return true
		} else {
...
func (g *genDeepCopy) doMap(t *types.Type, sw *generator.SnippetWriter) {
	...
	if ut.Key.IsAssignable() {
		dc, dci := deepCopyMethod(ut.Elem), deepCopyIntoMethod(ut.Elem)
		switch {
		case dc != nil || dci != nil:
...
func (g *genDeepCopy) doSlice(t *types.Type, sw *generator.SnippetWriter) {
	...
	sw.Do("*out = make($.|raw$, len(*in))\n", t)
	if deepCopyMethod(ut.Elem) != nil || deepCopyIntoMethod(ut.Elem) != nil {
		...
		sw.Do("for i := range *in {\n", nil)
		if uet.Kind == types.Slice || uet.Kind == types.Map || deepCopyMethod(ut.Elem) != nil || deepCopyIntoMethod(ut.Elem) != nil {
			sw.Do("if (*in)[i] != nil {\n", nil)
...
func (g *genDeepCopy) doStruct(t *types.Type, sw *generator.SnippetWriter) {
	...
	for _, m := range ut.Members {
		...
		dc, dci := deepCopyMethod(ft), deepCopyIntoMethod(ft)
		switch {
		case dc != nil || dci != nil:
			// Note: a DeepCopyInto exists because it is added if DeepCopy is manually defined
