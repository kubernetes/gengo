	...
	for _, i := range context.Inputs {
		...
		for {
			...
			for t, d := range newDefaulters {
				...
				if newCallTreeForType(existingDefaulters, newDefaulters, currentlyBuildingTypes).build(t, true) != nil {
...
func newCallTreeForType(existingDefaulters, newDefaulters defaulterFuncMap, currentlyBuildingTypes map[*types.Type]bool) *callTreeForType {
...
func (c *callTreeForType) build(t *types.Type, root bool) *callNode {
	...
	case types.Struct:
		for _, field := range t.Members {
			...
			r := newCallTreeForType(c.existingDefaulters, c.newDefaulters, c.currentlyBuildingTypes)
			if child := r.build(field.Type, false); child != nil {
				child.field = name
				parent.children = append(parent.children, *child)
			}
		}
	case types.Alias:
		r := newCallTreeForType(c.existingDefaulters, c.newDefaulters, c.currentlyBuildingTypes)
		if child := r.build(t.Underlying, false); child != nil {
			parent.children = append(parent.children, *child)
		}
	}
...
func (g *genDefaulter) GenerateType(c *generator.Context, t *types.Type, w io.Writer) error {
	...
	currentlyBuildingTypes := make(map[*types.Type]bool)
	callTree := newCallTreeForType(g.existingDefaulters, g.newDefaulters, currentlyBuildingTypes).build(t, true)
	if callTree == nil {
