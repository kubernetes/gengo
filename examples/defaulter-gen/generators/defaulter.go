...
var refRE = regexp.MustCompile(`^ref\((("(?P<pkg>.*)")\.)?(?P<ident>[^"]*)\)$`)
var refREPkgIndex = refRE.SubexpIndex("pkg")
var refREIdentIndex = refRE.SubexpIndex("ident")
...
//     false.
func parseAsRef(s string) (string, string, bool) {
	matches := refRE.FindStringSubmatch(s)
	if len(matches) < refREIdentIndex || matches[refREIdentIndex] == "" {
		return "", "", false
	}
	return matches[refREPkgIndex], matches[refREIdentIndex], true
}
...
func populateDefaultValue(node *callNode, t *types.Type, tags string, commentLines []string) *callNode {
	...
	if pkg, id, ok := parseAsRef(defaultString); ok {
		if len(pkg) > 0 {
			symbolReference = pkg + "." + id
		} else {
			symbolReference = id
		}
		defaultString = ""
