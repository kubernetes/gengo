...
func (b *Builder) AddDirRecursive(dir string) error {
	...
	fn := func(path string, info os.FileInfo, err error) error {
		if info != nil && info.IsDir() {
			...
			if rel != "" {
				// Make a pkg path.
				pkg := filepath.Join(canonicalizeImportPath(b.buildPackages[dir].ImportPath), rel)
...
func (b *Builder) AddDirTo(dir string, u *types.Universe) error {
	...
	return b.findTypesIn(importPathString(canonicalizeImportPath(b.buildPackages[dir].ImportPath)), u)
}
...
func canonicalizeImportPath(importPath string) string {
